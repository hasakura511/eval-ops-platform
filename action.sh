...
"$PY" ./tools/hint_eval extract --cache-dir "$CACHE_DIR" --out "$FEATURES_OUT"

echo "2) Join labels + features -> $LABELED_WITH_FEATURES_OUT"
"$PY" ./tools/hint_eval join --labeled "$LAB_PATH" --features "$FEATURES_OUT" --out "$LABELED_WITH_FEATURES_OUT"

echo "3) Baseline eval"
"$PY" ./tools/hint_eval eval --labeled "$LABELED_WITH_FEATURES_OUT" --config config/thresholds.yaml

echo "4) Fit thresholds -> $FITTED_CFG"
"$PY" ./tools/hint_eval fit --train "$LABELED_WITH_FEATURES_OUT" \
  --config-in config/thresholds.yaml \
  --config-out "$FITTED_CFG"

echo "5) Eval with fitted thresholds"
"$PY" ./tools/hint_eval eval --labeled "$LABELED_WITH_FEATURES_OUT" --config "$FITTED_CFG"
